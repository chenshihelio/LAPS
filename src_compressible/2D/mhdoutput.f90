module mhdoutput
    use mhdinit, only: xgrid, ygrid, nx, ny, nvar, &
        uu, uu_prim
    use parallel
    ...
    contains
        subroutine output_initialize
            ...
            out_size_full = [nx,ny,1,nvar]
            out_size_sub = [nx,yi_size(myid_i+1),1,nvar]
            out_start_ind = [0,yi_offset(myid_i+1),0,0]
        ...
        subroutine output_grid
            ...
            if (ipe ==0) then 
                open(10,file='grid.dat',form='unformatted')
                    write(10) real(nx,4), real(ny,4)
                    write(10) real(xgrid,4),real(ygrid,4)
                close(10)
            endif
        ...
        subroutine output_parallel_info
            ...
            if (ipe == 0) then 
                open(10, file='parallel_info.dat', form='unformatted')
                    write(10) real(npe,4),real(nvar,4)
                close(10)
            endif
